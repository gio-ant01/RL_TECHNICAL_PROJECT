...
def generate_launch_description():
    ...
    rviz_config_file = os.path.join(get_package_share_directory('escape_room'), 'rviz_conf', 'amcl_view.rviz')
    ...
    map_file_arg = DeclareLaunchArgument(
        "map_file",
        default_value=PathJoinSubstitution([FindPackageShare("escape_room"), "maps", "map.yaml"]),
    ...
    amcl_params_file_arg = DeclareLaunchArgument(
        ...
        default_value=PathJoinSubstitution(
            [FindPackageShare("escape_room"), "config", "fra2mo", "amcl.yaml"]
        ),
